...
use accounting::statistics::{TimePeriod, Statistics};
use error::{Error, ErrorKind};
use bot::email::EmailSender;
use config::Config;

pub fn report<'a, I>(commands: &mut I, config: &Config, registry: &Registry, user: UserId) -> Result<String, Error> where I: Iterator<Item=&'a str> {
...
impl<'r> ReportFactory<'r> {
    ...
    fn try_send_report(&self, config: &Config, time_period: &str, email: &str) -> Result<String, Error> {
        ...
        match report {
            Some(actual_report) => {
                let react_report = format!("{}", ::representation::ReactReportRepresentation::from(actual_report));
...
fn parse_year(time_period: &str) -> Result<TimePeriod, Error> {
    ...
    let invalid_date: Error = ErrorKind::InvalidDate.into();
    let end_of_year = ::dates::last_day_of_month(NaiveDate::from_ymd_opt(year, 12, 1).ok_or(invalid_date)?);
